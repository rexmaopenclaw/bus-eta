...
- **改法**：同公司都加站名 fallback — 同名站（去括號）+ 相距 < 150 米 → 當同一轉車站；同一 ID 優先，其次揀最近嗰條柱
- 跨公司原本已用站名匹配，而家統一改用同一邏輯（防重複 + 距離過濾）
- 實測：42C + 92 → 2 個共同站（彩虹轉車站-碧海樓、牛池灣轉車站-紅萼樓）
